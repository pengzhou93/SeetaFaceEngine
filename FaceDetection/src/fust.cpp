...
bool FuStDetector::LoadModel(const std::string & model_path) {
  ...
  if (!model_file.is_open()) {
    ...
    std::shared_ptr<cmss::fd::ModelReader> reader;
    std::shared_ptr<cmss::fd::Classifier> classifier;
    cmss::fd::ClassifierType classifier_type;
    ...
    for (int32_t i = 0; is_loaded && i < num_hierarchy_; i++) {
      ...
      for (int32_t j = 0; is_loaded && j < hierarchy_size; j++) {
        ...
        for (int32_t k = 0; is_loaded && k < num_stage; k++) {
          ...
          classifier_type = static_cast<cmss::fd::ClassifierType>(type_id);
...
std::vector<cmss::FaceInfo> FuStDetector::Detect(
    cmss::fd::ImagePyramid* img_pyramid) {
  float score;
  cmss::FaceInfo wnd_info;
  cmss::Rect wnd;
  float scale_factor = 0.0;
  const cmss::ImageData* img_scaled =
  ...
  std::vector<std::vector<cmss::FaceInfo> > proposals(hierarchy_size_[0]);
  std::shared_ptr<cmss::fd::FeatureMap> & feat_map_1 =
  ...
  std::vector<std::vector<cmss::FaceInfo> > proposals_nms(hierarchy_size_[0]);
  for (int32_t i = 0; i < hierarchy_size_[0]; i++) {
    cmss::fd::NonMaximumSuppression(&(proposals[i]),
  ...
  cmss::ImageData img = img_pyramid->image1x();
  cmss::Rect roi;
  ...
  for (int32_t i = 1; i < num_hierarchy_; i++) {
    ...
    for (int32_t j = 0; j < hierarchy_size_[i]; j++) {
      ...
      for (int32_t k = 0; k < num_stage_[cls_idx]; k++) {
        ...
        std::vector<cmss::FaceInfo> & bboxes = proposals[buf_idx[j]];
        int32_t bbox_idx = 0;
        ...
        if (k < num_stage_[cls_idx] - 1) {
          cmss::fd::NonMaximumSuppression(&(proposals[buf_idx[j]]),
...
std::shared_ptr<cmss::fd::ModelReader>
FuStDetector::CreateModelReader(cmss::fd::ClassifierType type) {
  std::shared_ptr<cmss::fd::ModelReader> reader;
  switch (type) {
  case cmss::fd::ClassifierType::LAB_Boosted_Classifier:
    reader.reset(new cmss::fd::LABBoostModelReader());
    break;
  case cmss::fd::ClassifierType::SURF_MLP:
    reader.reset(new cmss::fd::SURFMLPModelReader());
    break;
  default:
    break;
  }
...
std::shared_ptr<cmss::fd::Classifier>
FuStDetector::CreateClassifier(cmss::fd::ClassifierType type) {
  std::shared_ptr<cmss::fd::Classifier> classifier;
  switch (type) {
  case cmss::fd::ClassifierType::LAB_Boosted_Classifier:
    classifier.reset(new cmss::fd::LABBoostedClassifier());
    break;
  case cmss::fd::ClassifierType::SURF_MLP:
    classifier.reset(new cmss::fd::SURFMLP());
    break;
  default:
    break;
  }
...
std::shared_ptr<cmss::fd::FeatureMap>
FuStDetector::CreateFeatureMap(cmss::fd::ClassifierType type) {
  std::shared_ptr<cmss::fd::FeatureMap> feat_map;
  switch (type) {
  case cmss::fd::ClassifierType::LAB_Boosted_Classifier:
    feat_map.reset(new cmss::fd::LABFeatureMap());
    break;
  case cmss::fd::ClassifierType::SURF_MLP:
    feat_map.reset(new cmss::fd::SURFFeatureMap());
    break;
  default:
    break;
  }
...
void FuStDetector::GetWindowData(const cmss::ImageData & img,
    const cmss::Rect & wnd) {
  int32_t pad_left;
  int32_t pad_right;
  int32_t pad_top;
  ...
  cmss::ImageData src_img(roi.width, roi.height);
  cmss::ImageData dest_img(wnd_size_, wnd_size_);
  src_img.data = wnd_data_buf_.data();
  dest_img.data = wnd_data_.data();
  cmss::fd::ResizeImage(src_img, &dest_img);
}
