...
bool CompareBBox(const cmss::FaceInfo & a, const cmss::FaceInfo & b) {
  return a.score > b.score;
}

void NonMaximumSuppression(std::vector<cmss::FaceInfo>* bboxes,
  std::vector<cmss::FaceInfo>* bboxes_nms, float iou_thresh) {
  bboxes_nms->clear();
  std::sort(bboxes->begin(), bboxes->end(), cmss::fd::CompareBBox);
  ...
  while (!all_merged) {
    ...
    cmss::Rect select_bbox = (*bboxes)[select_idx].bbox;
    ...
    for (int32_t i = select_idx; i < num_bbox; i++) {
      ...
      cmss::Rect & bbox_i = (*bboxes)[i].bbox;
