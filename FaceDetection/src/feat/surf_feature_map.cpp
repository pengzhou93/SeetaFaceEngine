...
void SURFFeatureMap::ComputeGradientImages(const uint8_t* input) {
  int32_t len = width_ * height_;
  cmss::fd::MathFunction::UInt8ToInt32(input, img_buf_.data(), len);
...
#pragma omp for nowait
    for (int32_t r = 0; r < height_; r++) {
      ...
      *dest = ((*(src + 1)) - (*src)) << 1;
      cmss::fd::MathFunction::VectorSub(src + 2, src, dest + 1, len);
      dest += (width_ - 1);
      src += (width_ - 1);
      *dest = ((*src) - (*(src - 1))) << 1;
    }
...
void SURFFeatureMap::ComputeGradY(const int32_t* input) {
  ...
  int32_t len = width_;
  cmss::fd::MathFunction::VectorSub(input + width_, input, dy, len);
  cmss::fd::MathFunction::VectorAdd(dy, dy, dy, len);
...
#pragma omp for nowait
    for (int32_t r = 1; r < height_ - 1; r++) {
      ...
      cmss::fd::MathFunction::VectorSub(src + (width_ << 1), src, dest, len);
    }
  }
  int32_t offset = (height_ - 1) * width_;
  dy += offset;
  cmss::fd::MathFunction::VectorSub(input + offset, input + offset - width_,
    dy, len);
  cmss::fd::MathFunction::VectorAdd(dy, dy, dy, len);
}
...
void SURFFeatureMap::ComputeIntegralImages() {
  ...
  int32_t len = width_ * height_;
  cmss::fd::MathFunction::VectorAbs(grad_x_.data(), img_buf_.data(), len);
  FillIntegralChannel(img_buf_.data(), 1);
  cmss::fd::MathFunction::VectorAbs(grad_y_.data(), img_buf_.data(), len);
...
void SURFFeatureMap::Integral() {
  ...
  for (int32_t r = 0; r < height_ - 1; r++) {
    ...
    cmss::fd::MathFunction::VectorAdd(row1, row2, row2, len);
  }
...
void SURFFeatureMap::VectorCumAdd(int32_t* x, int32_t len,
  ...
  for (int32_t i = 0; i < cols; i++) {
    ...
    cmss::fd::MathFunction::VectorAdd(col1, col2, col2, num_channel);
  }
#endif
}
