...
void LABFeatureMap::ComputeIntegralImages(const uint8_t* input) {
  ...
  cmss::fd::MathFunction::UInt8ToInt32(input, int_img_.data(), len);
  cmss::fd::MathFunction::Square(int_img_.data(), square_int_img_.data(), len);
...
void LABFeatureMap::ComputeRectSum() {
  ...
  *rect_sum = *(int_img + (rect_height_ - 1) * width_ + rect_width_ - 1);
  cmss::fd::MathFunction::VectorSub(int_img + (rect_height_ - 1) * width_ +
...
#pragma omp for nowait
    for (int32_t i = 1; i <= height; i++) {
      ...
      *(dest++) = (*bottom_right) - (*top_right);
      cmss::fd::MathFunction::VectorSub(bottom_right + 1, top_right + 1, dest, width);
      cmss::fd::MathFunction::VectorSub(dest, bottom_left, dest, width);
      cmss::fd::MathFunction::VectorAdd(dest, top_left, dest, width);
    }
  }
}
