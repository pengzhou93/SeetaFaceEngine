...
#ifndef cmss_FD_UTIL_IMAGE_PYRAMID_H_
#define cmss_FD_UTIL_IMAGE_PYRAMID_H_
...
static void ResizeImage(const cmss::ImageData & src, cmss::ImageData* dest) {
...
class ImagePyramid {
 public:
  ...
  inline cmss::ImageData image1x() {
    cmss::ImageData img(width1x_, height1x_, 1);
    img.data = buf_img_;
    return img;
  }

  const cmss::ImageData* GetNextScaleImage(float* scale_factor = nullptr);
 ...
 private:
  ...
  cmss::ImageData img_scaled_;
};

}  // namespace fd
}  // namespace cmss

#endif  // cmss_FD_UTIL_IMAGE_PYRAMID_H_
