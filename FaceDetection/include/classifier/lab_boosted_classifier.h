...
#ifndef cmss_FD_CLASSIFIER_LAB_BOOSTED_CLASSIFIER_H_
#define cmss_FD_CLASSIFIER_LAB_BOOSTED_CLASSIFIER_H_
...
class LABBoostedClassifier : public Classifier {
 public:
  ...
  inline virtual cmss::fd::ClassifierType type() {
    return cmss::fd::ClassifierType::LAB_Boosted_Classifier;
  }

  void AddFeature(int32_t x, int32_t y);
  void AddBaseClassifier(const float* weights, int32_t num_bin, float thresh);

  inline virtual void SetFeatureMap(cmss::fd::FeatureMap* featMap) {
    feat_map_ = dynamic_cast<cmss::fd::LABFeatureMap*>(featMap);
  }
 ...
 private:
  ...
  std::vector<cmss::fd::LABFeature> feat_;
  std::vector<std::shared_ptr<cmss::fd::LABBaseClassifier> > base_classifiers_;
  cmss::fd::LABFeatureMap* feat_map_;
  bool use_std_dev_;
};
...
#endif  // cmss_FD_CLASSIFIER_LAB_BOOSTED_CLASSIFIER_H_
