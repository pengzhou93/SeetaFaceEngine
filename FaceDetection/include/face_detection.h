...
#ifndef cmss_FACE_DETECTION_H_
#define cmss_FACE_DETECTION_H_
...
class FaceDetection {
 public:
  cmss_API explicit FaceDetection(const char* model_path);
  cmss_API ~FaceDetection();
   ...
   */
  cmss_API std::vector<cmss::FaceInfo> Detect(const cmss::ImageData & img);
   ...
   * be ignored.
   */
  cmss_API void SetMinFaceSize(int32_t size);
   ...
   */
  cmss_API void SetMaxFaceSize(int32_t size);
   ...
   */
  cmss_API void SetImagePyramidScaleFactor(float factor);
   ...
   */
  cmss_API void SetWindowStep(int32_t step_x, int32_t step_y);
   ...
   */
  cmss_API void SetScoreThresh(float thresh);
